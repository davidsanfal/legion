...
class Omnidirectional(Legionary):
    ...
    def calculate_in_speeds(self):
        self._speed.motor0_in = -self.speed.x_in*sin(pi/3) + (self.speed.y_in*cos(pi/3)) + self.arm*self.speed.w_in
        self._speed.motor1_in = -self.speed.y_in + self.arm*self.speed.w_in
        self._speed.motor2_in = self.speed.x_in*sin(pi/3) + (self.speed.y_in*cos(pi/3)) + self.arm*self.speed.w_in

    def calculate_out_speeds(self):
        self._speed.motor0_out = -self.speed.x_out*sin(pi/3) + (self.speed.y_out*cos(pi/3)) + self.arm*self.speed.w_out
        self._speed.motor1_out = -self.speed.y_out + self.arm*self.speed.w_out
        self._speed.motor2_out = self.speed.x_out*sin(pi/3) + (self.speed.y_out*cos(pi/3)) + self.arm*self.speed.w_out

    def read_speeds(self):
        self._speed.motor0 = self._speed.motor0_in
        self._speed.motor1 = self._speed.motor1_in
        self._speed.motor2 = self._speed.motor2_in
